...
def _apply_llm_output(parsed: dict, state) -> dict:
    ...
    if "beta" in action and state.grayness > GRAYNESS_GATE:
...
class LLMController:
    ...
    def _fallback(self, state) -> dict:
        ...
        if self._fb_phase in ("sharpening", "converge") and state.grayness > GRAYNESS_GATE:
    ...
    def _merge_phase_label(self, state, parsed: Optional[dict]) -> dict:
        """Legacy phase-label mode for ablation comparison."""
        ...
        if proposed in ("sharpening", "converge") and state.grayness > GRAYNESS_GATE:
    ...
    def __call__(self, state, rho):
        ...
        if state.grayness > GRAYNESS_GATE and state.beta > 8.0:
            return {"beta": 8.0}
